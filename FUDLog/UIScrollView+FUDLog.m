//
//  UIScrollView+FUDLog.m
//  FUDLog
//
//  Created by lanfudong on 2020/2/29.
//

#import "UIScrollView+FUDLog.h"
#import "NSObject+FUDLog.h"
#import <objc/runtime.h>

@implementation UIScrollView (FUDLog)
...
- (void)fud_tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSLog(@"FUDLog: [%@_didSelectCell_%@_atIndexPath_(%ld,%ld)_onDelegate_%@", [self class], [cell class], (long)indexPath.section, (long)indexPath.row, [delegate class]);
...
- (void)fud_collectionView:(UICollectionView *)collection didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSLog(@"FUDLog: [%@_didSelectItem_%@_atIndexPath_(%ld,%ld)_onDelegate_%@", [self class], [cell class], (long)indexPath.section, (long)indexPath.row, [delegate class]);
